...
pub async fn exec(bin: SpecBin, args: &[OsString], spec: Option<&Spec>) -> Result<()> {
    ...
    let (configured, configured_on_fail) = match spec {
        Some(v) => (Some(v.to_owned()), OnFail::default()),
        None => match Spec::parse(true, Some(bin_default_spec.name)).await? {
            Some(manifest) => (Some(manifest.spec), manifest.on_fail),
            None => (None, OnFail::default()),
        },
    };

    let (spec, on_fail) = match configured {
        Some(configured) if configured.name != bin_default_spec.name => {
            ...
            let transparent = matches!(
                bin,
                SpecBin::Npm | SpecBin::Npx | SpecBin::Pnpx | SpecBin::Pnx | SpecBin::Bunx
            ) || first_arg == Some("init")
                || matches!(bin_default_spec.name, SpecName::Yarn | SpecName::Pnpm)
                    && first_arg == Some("dlx");

            if !(disable_strict || transparent) {
                match configured_on_fail {
                    ...
                    OnFail::Download | OnFail::Error => {
                        bail!(
                            "{bin} is not available in the configured package manager {configured}",
                        );
                    }
            ...
            (bin_default_spec, OnFail::default())
        }

        Some(configured) => (configured, configured_on_fail),
        None => (bin_default_spec, OnFail::default()),
    };
